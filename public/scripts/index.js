...
$("form").submit(function(){
	...
	if ($("#fecha_inicio_habilitado").val()<=$("#fecha_actual").val() && $("#fecha_fin_habilitado").val()>=$("#fecha_actual").val()) {
		...
		if( !confirm("¡Mensaje! : Esta seguro de realizar la impresión?. Se lo cobrara "+$("#cobro_impresion").val() +" Bs. por la impresión") ) 
		return false;
	else 
		return true;	
	
	}else{
		alert("¡Mensaje!: Estimado Usuario, solo esta habilitado la impresión hasta el  "+$("#fecha_fin_habilitado").val()+". Comuniquese con el administrador del sistema. ");
		return false;
	}
	};
});
...
$("#filtro").click(function(){
	$("#sw").val(1);
	$("#testForm").attr("target","_self");
	$("#testForm").attr("target","_self");
	$("#testForm").attr("action","");
});
$("#pdfSinH").click(function(){
	$("#sw").val(0);
	$("#testForm").attr("target","_blank");
	$("#testForm").attr("action","/bases/pdfsinh");
});

$("#pdfConH").click(function(){
	$("#sw").val(0);
	$("#testForm").attr("target","_blank");
	$("#testForm").attr("action","/bases/pdfconh");
});
